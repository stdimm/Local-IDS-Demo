...
class NSLKDDTestNet(torch.nn.Module):
    def __init__(self):
        ...
        '''
        self.conv1 = torch.nn.Conv1d(kernel_size=122, out_channels=64, in_channels=1, padding=61)
        self.conv2 = torch.nn.Conv1d(in_channels=32, kernel_size=2, out_channels=64)
        self.pool = torch.nn.MaxPool1d(5)
        self.norm1 = torch.nn.BatchNorm1d(num_features=64, eps=0.001, momentum=0.99)
        self.norm2 = torch.nn.BatchNorm1d(num_features=1, eps=0.001, momentum=0.99)
        self.lstm1 = torch.nn.LSTM(hidden_size=64, bidirectional=True, input_size=24, num_layers=1, batch_first=True)
        self.lstm2 = torch.nn.LSTM(hidden_size=128, bidirectional=True, input_size=25, num_layers=1, batch_first=True)
    ...
    def forward(self, x):
        ...
        #y = x.reshape((-1, 1, 128))
        #print(y.shape)
        #z = torch.reshape(x, ( , 1, 128))
        #x = torch.reshape(x, (128, 1, 128))
        #print(x.shape)
        #print(z.shape)
